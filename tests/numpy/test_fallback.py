...
def test_manual_fallabck(nps_app_inst):
    assert nps_app_inst is not None

    passed = []
    failed = []

    for manual_func in settings.manually_tested_fallback:
        try:
            eval(manual_func + "_manual_test")()
            passed.append(manual_func)
        except BaseException:
            failed.append(manual_func)
...
if __name__ == "__main__":
    ...
    test_manual_fallabck(nps_app_inst)
